...
public class ConversationManager
{
    ...
    public async Task UpdateChatMessage(AIChatMessage message, bool isUpdate)
    {
        ...
        if (isUpdate)
        {
            ...
            _connection.Execute(updateQuery, message.IsServer, message.IsUser, message.Message, message.Idx);
        }
        else
        {
    ...
    public void AddChatMessage(int conversationIdx, AIChatMessage message)
    {
        var tableName = $"Messages_{conversationIdx}";
        var insertQuery = $@"
            INSERT INTO {tableName} (IsServer, IsUser, Message)
            VALUES (?, ?, ?)";

        _connection.Execute(insertQuery, message.IsServer, message.IsUser, message.Message);
